...
const FREE_GIFT = { id: 99, name: "Wireless Mouse", price: 0, isGift: true };
...
const ProductCart = () => {
  ...
  const subtotal = cart.reduce(
    (sum, item) =>
      item.id === FREE_GIFT.id ? sum : sum + item.price * item.qty,
    0
  );
  ...
  const handleAddToCart = (product) => {
    ...
    // Handle Quatntiy change to cart
  ...
  // Handle Remove quatantity change to cart
        ...
        <div className="w-full bg-gray-200 rounded h-5 border border-gray-300 overflow-hidden">
          <div
            className={`h-5 transition-all duration-300 ${
              percent === 100 ? "bg-blue-600" : "bg-blue-600"
            }`
          }
          style={{width: `${percent}%`}}
          ></div>
